...
def main():
    ...
    for k, seed in enumerate(tqdm.tqdm(seeds)):
        ...
        train_X, test_X = np.array(train_X).astype(np.float64), np.array(test_X).astype(np.float64)
        ...
        print("train target = {}".format(train_targets))
        test_groups_scores = predictor_a.predict(test_X).squeeze()
        #test_groups_scores = np.random.randint(0, 2, size=test_groups_scores.shape)
        
        #from sklearn.metrics import accuracy_score
        #acc = accuracy_score(test_groups_scores, test_groups)
        ...
        if k == 3:
            ...
            plt.savefig("law_images/svd_{}_{}.png".format(50, 150))
